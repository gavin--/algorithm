...
class MinQueue {
 public:
  auto empty() const { return left_.empty() && right_.empty(); }

  auto size() const { return left_.size() + right_.size(); }
  ...
  void push(T value) {
    left_.push_back({left_.empty()
                         ? value
                         : std::min(value, left_.back().second, Compare()),
                     std::move(value)});
  }
